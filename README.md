...
| **Иконка** | PNG/BMP → **Ч/Б 1-bit** или **RGB565**; масштаб; `drawBitmap` / `pushImage` |
...
5. Иконки: **Иконка PNG/BMP** (или инструмент «Иконка») → масштаб/порог в свойствах → в коде массив + `drawBitmap`
6. **Код экрана** — скопировать `drawGui()` в скетч
7. **Сохранить** — JSON, чтобы не потерять работу после обновления страницы
...
│   ├── bitmap.js       # PNG/BMP → 1-bit drawBitmap
